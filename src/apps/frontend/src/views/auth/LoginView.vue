<template>
  <div class="flex w-full bg-surface-50 dark:bg-surface-950 min-h-screen">
    <div class="bg-surface-0 dark:bg-surface-950 w-full md:w-6/12 px-8 py-20 md:px-12 lg:px-20 flex flex-col gap-12 rounded min-h-screen">
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-2">
          <div class="text-surface-900 dark:text-surface-0 text-2xl font-semibold leading-tight">Bienvenido de nuevo</div>

        </div>
      </div>
      <form @submit.prevent="handleLogin" class="flex flex-col gap-6">
        <div class="flex flex-col gap-2">
          <label for="email2" class="text-surface-900 dark:text-surface-0 font-medium">Correo electrónico</label>
          <InputText id="email2" v-model="email" type="email" placeholder="Correo electrónico" class="w-full p-3 rounded-md shadow-sm" />
        </div>
        <div class="flex flex-col gap-2">
          <label for="password2" class="text-surface-900 dark:text-surface-0 font-medium">Contraseña</label>
          <Password id="password2" v-model="password" placeholder="Contraseña" :toggleMask="true" :feedback="false" input-class="w-full!" />
        </div>
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <Checkbox id="rememberme2" v-model="checked2" :binary="true" />
            <label for="rememberme2" class="text-surface-900 dark:text-surface-0">Recordarme</label>
          </div>
          <a class="text-surface-500 dark:text-surface-400 font-medium cursor-pointer hover:text-surface-600 dark:hover:text-surface-300">¿Olvidaste tu contraseña?</a>
        </div>
        <Button type="submit" :label="loading ? 'Entrando...' : 'Iniciar sesión'" class="w-fit px-12!" :loading="loading" />
      </form>
    </div>
    <div
      ...
      style="
        background:
          ...
          url('https://fqjltiegiezfetthbags.supabase.co/storage/v1/object/public/block.images/blocks/signin/signin-split.jpg') lightgray 50% / cover no-repeat;
