<template>
  <div class="p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <Button label="Nuevo Alumno" icon="pi pi-plus" @click="router.push('/alumnos/nuevo')" />
    </div>
    ...
    <DataTable :value="items" :loading="loading" :paginator="true" :rows="limit" :totalRecords="total" lazy @page="onPage">
      ...
      <Column header="Acciones">
        <template #body="{ data }">
          <Button icon="pi pi-pencil" class="p-button-text" @click="router.push(`/alumnos/${data.id}/editar`)" />
    ...
    <Dialog v-model:visible="deleteDialog" header="Confirmar eliminación" modal>
      <p class="m-0">¿Estás seguro de que quieres eliminar el alumno <strong>{{ itemToDelete?.nombre }} {{ itemToDelete?.apellidos }}</strong>?</p>
...
async function cargar() {
  loading.value = true;
  try {
    const { data } = await api.get('/alumnos', { params: { page: page.value, limit } });
...
async function eliminar() {
  ...
  await api.delete(`/alumnos/${itemToDelete.value.id}`);
