...
export const ImportarMatriculasPage: React.FC = () => {
  const [archivos, setArchivos] = useState<File[]>([]);
  const [informe, setInforme] = useState<InformeImportacionMatriculas | null>(null);
  ...
  const importar = async (e: React.FormEvent) => {
    ...
    setInforme(null);
    try {
      const res = await matriculasService.importar(archivos);
      setInforme(res);
    } catch (err) {
      const detail =
    ...
    <div className="page">
      ...
      <form onSubmit={importar} className="form-card">
        ...
        <p style={{ color: '#6e6e73', fontSize: '0.875rem', marginTop: '0.5rem' }}>
          ...
          <code>alumno_username,curso_academico,asignatura_codigo,n_matricula</code>.
          Los alumnos deben haberse importado antes.
        </p>
      ...
      {informe && (
        <div className="form-card" style={{ marginTop: '1rem' }}>
          ...
          <p>
            <strong>{informe.matriculas_creadas}</strong> matrículas (header) ·{' '}
            <strong>{informe.asignaturas_matriculadas_creadas}</strong> asignaturas matriculadas (detalle) ·{' '}
