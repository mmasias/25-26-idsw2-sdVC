...
const ImportarAulasView: React.FC = () => {
  ...
  const parseCSV = (text: string) => {
    ...
    const data = lines.slice(1).map(line => {
      ...
      return {
        codigo: parts[0]?.trim(),
        nombre: parts[1]?.trim(),
        capacidad: parts[2]?.trim(),
        ubicacion: parts[3]?.trim() || ''
      };
  ...
  const handleImportar = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const result = await aulasService.importAulas(data);
        
        const summary = `✅ IMPORTACIÓN COMPLETADA!\n\n📊 Procesados: ${data.length}\n✨ Éxito: ${result.success}\n❌ Fallidos: ${result.failed}`;
        alert(summary);

        if (result.failed > 0) {
          setMensajeError(`Se importaron ${result.success} aulas, pero ${result.failed} fallaron. Revise los códigos duplicados.`);
        } else {
          setMensajeExito(`🎉 ¡Importación exitosa! Se procesaron ${result.success} aulas correctamente.`);
        }
      } catch (err) {
    ...
    <div style={{ 
      ...
      <div style={{ width: '650px', background: '#ececec', padding: '0px' }}>
        ...
        <div style={{ border: '1px solid #cfcfcf', padding: '18px', margin: '0 20px 28px 20px', background: '#ededed' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '22px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <label style={{ fontSize: '18px', cursor: 'pointer' }}>Forzar actualización de datos</label>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', paddingBottom: '30px' }}>
          ...
          >
            {loading ? 'Procesando...' : 'Importar aulas'}
          </button>
          <button 
...
export default ImportarAulasView;
