...
const ImportarAsignaturasView: React.FC = () => {
  ...
  // Checkboxes
  const [validarGrados, setValidarGrados] = useState(true);
  const [creditosDefecto, setCreditosDefecto] = useState(true);
  ...
  const parseCSV = (text: string) => {
    ...
    // Assume header: codigo, nombre, creditos, gradoCodigo
    const data = lines.slice(1).map(line => {
      const parts = line.split(/[,;]/);
      return {
        codigo: parts[0]?.trim(),
        nombre: parts[1]?.trim(),
        creditos: parseInt(parts[2]?.trim()) || (creditosDefecto ? 6 : 0),
        gradoCodigo: parts[3]?.trim()
      };
    }).filter(item => item.codigo && item.nombre && item.gradoCodigo);
  ...
  const handleImportar = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (data.length === 0) {
          setMensajeError('❌ El archivo no contiene datos válidos o faltan campos obligatorios (Código, Nombre, Grado).');
          setLoading(false);
          return;
        }

        const result = await asignaturasService.importAsignaturas(data);
        ...
        if (result.failed > 0) {
          setMensajeError(`Se importaron ${result.success} asignaturas, pero ${result.failed} fallaron. Verifique los códigos duplicados o la existencia de los grados.`);
        } else {
          setMensajeExito(`🎉 ¡Importación exitosa! Se procesaron ${result.success} asignaturas correctamente.`);
        }
      } catch (err) {
        setMensajeError('❌ Error al procesar la importación. Verifique la conexión con el servidor.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div style={{ 
      ...
      <div style={{ width: '650px', background: '#ececec', padding: '0px', border: '1px solid #cfcfcf' }}>
        ...
        <div style={{ border: '1px solid #cfcfcf', padding: '18px', margin: '0 20px 28px 20px', background: '#ededed' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '22px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="checkbox" checked={validarGrados} onChange={(e) => setValidarGrados(e.target.checked)} style={{ width: '16px', height: '16px', cursor: 'pointer' }} />
              <label style={{ fontSize: '18px', cursor: 'pointer' }}>Validar grados existentes</label>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="checkbox" checked={creditosDefecto} onChange={(e) => setCreditosDefecto(e.target.checked)} style={{ width: '16px', height: '16px', cursor: 'pointer' }} />
              <label style={{ fontSize: '18px', cursor: 'pointer' }}>Créditos por defecto <small style={{fontStyle:'italic', color: '#666'}}>(6)</small></label>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="checkbox" style={{ width: '16px', height: '16px', cursor: 'pointer' }} />
              <label style={{ fontSize: '18px', cursor: 'pointer' }}>Crear grados automáticos</label>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', paddingBottom: '30px' }}>
          ...
          >
            {loading ? 'Procesando...' : 'Importar asignaturas'}
          </button>
          <button 
...
export default ImportarAsignaturasView;
